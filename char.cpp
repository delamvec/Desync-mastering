...
CHARACTER::CHARACTER()
{
	...
	m_stateSyncing.Set(this, &CHARACTER::BeginStateEmpty, &CHARACTER::StateSyncing, &CHARACTER::EndStateEmpty); // PHASE 3: Position sync state
...
void CHARACTER::Initialize()
{
	...
	m_dwLastComboTime = 0;
	m_bComboIndex = 0;
	m_dwLastComboTarget = 0;         // PHASE 2: Initialize combo target tracking
	m_dwLastKnockbackTime = 0;       // KNOCKBACK IMMUNITY: Initialize knockback timestamp
	m_iComboHackCount = 0;
	m_dwSkipComboAttackByTime = 0;

	// PHASE 3: Initialize position syncing variables
	m_dwSyncStartTime = 0;
	m_dwSyncDuration = 0;
	m_bIsSyncing = false;
	...
	// PHASE 1: Initialize server-side position tracking
	m_dwSyncStartTime = 0;
	m_dwSyncDuration = 0;
	m_bIsSyncing = false;
}
...
void CHARACTER::UpdateCharacter(DWORD dwPulse)
{
	...
	// PHASE 1: Update sync state if active
	if (m_bIsSyncing)
	{
		StateSyncing();
	}
...
// PHASE 1: Server-side position tracking - gradual position update
void CHARACTER::StateSyncing()
{
	...
	DWORD dwElapsed = get_dword_time() - m_dwSyncStartTime;
	float fProgress = (float)dwElapsed / (float)m_dwSyncDuration;

	// Clamp to max 1.0 (100%)
	if (fProgress > 1.0f)
		fProgress = 1.0f;

	// Calculate current position based on progress
	int x = (int)((float)(m_posDest.x - m_posStart.x) * fProgress + m_posStart.x);
	int y = (int)((float)(m_posDest.y - m_posStart.y) * fProgress + m_posStart.y);

	// Update server's knowledge of player position
	Sync(x, y);

	// If reached destination, stop syncing
	if (fProgress >= 1.0f)
	{
		m_bIsSyncing = false;
	}
...
// PHASE 1: Start server-side position tracking
bool CHARACTER::BlendSync(long x, long y, unsigned int duration)
{
	// If already at destination, no need to sync
	if (GetX() == x && GetY() == y)
		return false;

	// If duration is 0, teleport immediately
	if (duration == 0)
	{
		Sync(x, y);
		return false;
...
void CHARACTER::ResetPoint(int iLv)
{
	...
	//PointChange(POINT_STAT, ((MINMAX(1, iLv, 200) - 1) * 2) + GetPoint(POINT_LEVEL_STEP) - GetPoint(POINT_STAT));

	if(iLv <= 200)
		PointChange(POINT_STAT, ((MINMAX(1, iLv, 200) - 1) * 2) + GetPoint(POINT_LEVEL_STEP) - GetPoint(POINT_STAT));
	else
		PointChange(POINT_STAT, 400 - GetPoint(POINT_STAT));
...
// ============================================================
// KNOCKBACK IMMUNITY - 2 second cooldown
// ============================================================
void CHARACTER::SetLastKnockbackTime(DWORD time)
{
	m_dwLastKnockbackTime = time;
}

DWORD CHARACTER::GetLastKnockbackTime() const
{
	return m_dwLastKnockbackTime;
}
...
void CHARACTER::IncrementComboIndex()
{
	m_bComboIndex++;
	if (m_bComboIndex > 3)
	{
		m_bComboIndex = 0;  // Reset to 0 after 4th hit (index 3)
	}
	sys_log(0, "COMBO_INCREMENT: %s combo index now %u", GetName(), m_bComboIndex);
}

void CHARACTER::ResetComboIndex()
{
	sys_log(0, "COMBO_RESET: %s combo index reset from %u to 0", GetName(), m_bComboIndex);
	m_bComboIndex = 0;
}

void CHARACTER::SetComboIndex(BYTE idx)
{
	m_bComboIndex = (idx > 3) ? 0 : idx;
	sys_log(0, "COMBO_SET: %s combo index set to %u", GetName(), m_bComboIndex);
}

DWORD CHARACTER::GetLastComboTarget() const
{
	return m_dwLastComboTarget;
}

void CHARACTER::SetLastComboTarget(DWORD vid)
{
	if (m_dwLastComboTarget != vid)
	{
		sys_log(0, "COMBO_TARGET: %s changed target from %u to %u",
		        GetName(), m_dwLastComboTarget, vid);
	}
	m_dwLastComboTarget = vid;
}
