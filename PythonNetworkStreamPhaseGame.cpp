...
bool CPythonNetworkStream::RecvAttackPacket()
{
	TPacketGCAttack kPacket;
	if (!Recv(sizeof(kPacket), &kPacket))
	{
		Tracen("CPythonNetworkStream::RecvAttackPacket Error");
		return false;
	}

	CPythonCharacterManager& rkChrMgr = CPythonCharacterManager::Instance();

	// Find attacker and victim instances
	CInstanceBase* pkAttacker = rkChrMgr.GetInstancePtr(kPacket.dwAttackerVID);
	CInstanceBase* pkVictim = rkChrMgr.GetInstancePtr(kPacket.dwVictimVID);

	if (!pkAttacker || !pkVictim)
		return true;

	// ============================================================
	// SERVER-AUTHORITATIVE KNOCKBACK APPLICATION
	// ============================================================

	// Apply server-calculated knockback to victim
	if (kPacket.dwDuration > 0 && kPacket.bComboIndex == 3)
	{
		// Convert from server POSITION to client pixel position
		long lDestX = kPacket.lDestX;
		long lDestY = kPacket.lDestY;
		__GlobalPositionToLocalPosition(lDestX, lDestY);

		// Apply smooth knockback animation
		TPixelPosition kDest;
		kDest.x = (float)lDestX;
		kDest.y = (float)lDestY;
		kDest.z = 0.0f;

		// Try to use duration from server
		// Option A: Use GetGraphicThingInstanceRef (has 2-param version)
		float fBlendTime = (float)kPacket.dwDuration / 1000.0f; // Convert ms to seconds

		try
		{
			// This should work if CActorInstance::SetBlendingPosition(pos, duration) exists
			pkVictim->GetGraphicThingInstanceRef().SetBlendingPosition(kDest, fBlendTime);
		}
		catch(...)
		{
			// Fallback: Use default blending time
			// This is safe but may not match server timing exactly
			pkVictim->SetBlendingPosition(kDest);
		}
	}
...
bool CPythonNetworkStream::SendAttackPacket(UINT uMotAttack, DWORD dwVIDVictim)
{
	...
	// PHASE 2 FULL: Get victim position and knockback info
	kPacketAtk.lStartX = 0;
	kPacketAtk.lStartY = 0;
	kPacketAtk.lDestX = 0;
	kPacketAtk.lDestY = 0;
	kPacketAtk.dwDuration = 0;
	kPacketAtk.dwTimestamp = 0;
	kPacketAtk.bComboIndex = 0;

	try
	{
		// Get victim instance
		CInstanceBase* pkVictim = CPythonCharacterManager::Instance().GetInstancePtr(dwVIDVictim);
		if (pkVictim)
		{
			// Get victim's CURRENT position (before attack)
			TPixelPosition kPos;
			pkVictim->NEW_GetPixelPosition(&kPos);

			if ((kPos.x != 0 || kPos.y != 0) && abs(kPos.x) < 1000000 && abs(kPos.y) < 1000000)
			{
				kPacketAtk.lStartX = (long)kPos.x;
				kPacketAtk.lStartY = (long)kPos.y;
				__LocalPositionToGlobalPosition(kPacketAtk.lStartX, kPacketAtk.lStartY);

				// Send victim current position
				// Server will calculate knockback destination
				kPacketAtk.lDestX = kPacketAtk.lStartX;
				kPacketAtk.lDestY = kPacketAtk.lStartY;
				kPacketAtk.dwDuration = 0;  // Server calculates duration
			}
		}

		kPacketAtk.dwTimestamp = ELTimer_GetServerMSec();
		kPacketAtk.bComboIndex = 0;
	}
	catch(...)
	{
