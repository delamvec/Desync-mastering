...
class CHARACTER : public CEntity, public CFSM, public CHorseRider
{
	...
	protected:
		...
		CStateTemplate<CHARACTER>	m_stateSyncing;   // PHASE 3: Concurrent state for position sync
	...
	public:
		...
		// PHASE 1: Server-side position tracking
		void				StateSyncing();
		bool				BlendSync(long x, long y, unsigned int duration);
		void				StopSyncState() { m_bIsSyncing = false; }
		bool				IsSyncing() const { return m_bIsSyncing; }
...
//		bool			IsPet() const		{ return m_bCharType == CHAR_TYPE_PET; }
	...
	public:
		...
		void IncrementComboIndex();           // PHASE 2: Increment combo 0→1→2→3→0
		void ResetComboIndex();                // PHASE 2: Reset combo to 0
		void SetComboIndex(BYTE idx);          // PHASE 2: Set specific combo index

		// Combo target tracking
		DWORD GetLastComboTarget() const;      // PHASE 2: Get last victim VID
		void SetLastComboTarget(DWORD vid);    // PHASE 2: Set last victim VID

		// Knockback immunity tracking (2 second cooldown)
		DWORD GetLastKnockbackTime() const;    // Get timestamp of last knockback received
		void SetLastKnockbackTime(DWORD time); // Set timestamp when knockback applied
	...
	protected:
		...
		BYTE m_bComboIndex;
		DWORD m_dwLastComboTarget;          // PHASE 2: Track last victim VID for combo reset
		DWORD m_dwLastKnockbackTime;        // KNOCKBACK IMMUNITY: Timestamp of last knockback received
