...
void CInputMain::Attack(LPCHARACTER ch, const BYTE header, const char* data)
{
	...
	// ═══════════════════════════════════════════════════════════════════
	sys_err("═══════════════════════════════════════════════════════════");
	sys_err("DEBUG_ATTACK_1: Attack() function CALLED!");
	sys_err("DEBUG_ATTACK_1: ch=%p header=%d data=%p", ch, header, data);

	if (NULL == ch)
	{
		sys_err("DEBUG_ATTACK_1: ch is NULL - RETURNING!");
		return;
	}

	sys_err("DEBUG_ATTACK_1: Character name: %s (VID=%u)", ch->GetName(), (DWORD)ch->GetVID());
	...
	switch (header)
	{
		case HEADER_CG_ATTACK:
			sys_err("DEBUG_ATTACK_2: Entered HEADER_CG_ATTACK case!");
			{
				if (NULL == ch->GetDesc())
				{
					sys_err("DEBUG_ATTACK_2: ch->GetDesc() is NULL - RETURNING!");
					return;
				}

				const TPacketCGAttack* const packMelee = reinterpret_cast<const TPacketCGAttack*>(data);

				sys_err("DEBUG_ATTACK_3: TPacketCGAttack received:");
				sys_err("DEBUG_ATTACK_3:   dwVID=%u bType=%d", packMelee->dwVID, packMelee->bType);
				sys_err("DEBUG_ATTACK_3:   lStartX=%ld lStartY=%ld", packMelee->lStartX, packMelee->lStartY);
				sys_err("DEBUG_ATTACK_3:   lDestX=%ld lDestY=%ld", packMelee->lDestX, packMelee->lDestY);
				sys_err("DEBUG_ATTACK_3:   dwDuration=%u bComboIndex=%d", packMelee->dwDuration, packMelee->bComboIndex);

				ch->GetDesc()->AssembleCRCMagicCube(packMelee->bCRCMagicCubeProcPiece, packMelee->bCRCMagicCubeFilePiece);

				LPCHARACTER	victim = CHARACTER_MANAGER::instance().Find(packMelee->dwVID);

				sys_err("DEBUG_ATTACK_4: Finding victim VID=%u result=%p", packMelee->dwVID, victim);

				if (NULL == victim || ch == victim)
				{
					sys_err("DEBUG_ATTACK_4: Victim is NULL or same as attacker - RETURNING!");
					return;
				}

				sys_err("DEBUG_ATTACK_4: Victim found: %s (VID=%u)", victim->GetName(), (DWORD)victim->GetVID());
				...
				// ============================================================
				// PHASE 2 FULL: Server-authoritative combat with combo tracking
				// ============================================================

				sys_log(0, "ATTACK_FULL: %s attacks %s (type=%d)",
				        ch->GetName(), victim->GetName(), packMelee->bType);

				// ============================================================
				// SERVER-AUTHORITATIVE COMBO INDEX PROGRESSION
				// ============================================================

				DWORD dwNow = get_dword_time();
				DWORD victimVID = victim->GetVID();

				sys_err("DEBUG_ATTACK_5: ═══ SERVER COMBO TRACKING ═══");
				sys_err("DEBUG_ATTACK_5:   Current combo index: %u", ch->GetComboIndex());
				sys_err("DEBUG_ATTACK_5:   Last target: %u (attacking: %u)", ch->GetLastComboTarget(), victimVID);
				sys_err("DEBUG_ATTACK_5:   Time since last: %u ms", dwNow - ch->GetLastComboTime());

				// Check if we should reset combo (target changed or timeout)
				bool bResetCombo = false;

				// CONDITION 1: Target changed
				if (ch->GetLastComboTarget() != victimVID)
				{
					sys_log(0, "COMBO: Target changed from %u to %u - RESET",
					        ch->GetLastComboTarget(), victimVID);
					bResetCombo = true;
				}

				// CONDITION 2: Timeout (more than 2 seconds since last hit)
				else if (ch->GetLastComboTime() > 0 &&
				         (dwNow - ch->GetLastComboTime()) > 2000)
				{
					sys_log(0, "COMBO: Timeout %ums - RESET",
					        dwNow - ch->GetLastComboTime());
					bResetCombo = true;
				}

				// Apply reset or increment
				if (bResetCombo)
				{
					ch->ResetComboIndex();
					sys_log(0, "COMBO: Index reset to 0");
				}

				// Get current server combo index (BEFORE increment)
				BYTE serverComboIndex = ch->GetComboIndex();

				// Increment for next hit
				ch->IncrementComboIndex();
				ch->SetLastComboTarget(victimVID);
				ch->SetLastComboTime(dwNow);

				sys_err("DEBUG_ATTACK_6: ═══ COMBO RESULT ═══");
				sys_err("DEBUG_ATTACK_6:   THIS hit index: %u (1st=0, 2nd=1, 3rd=2, 4th=3)", serverComboIndex);
				sys_err("DEBUG_ATTACK_6:   NEXT hit will be: %u", ch->GetComboIndex());
				sys_err("DEBUG_ATTACK_6:   KNOCKBACK? %s", serverComboIndex == 3 ? "YES - 4th HIT!" : "NO");

				sys_log(0, "COMBO: %s hits %s - Combo=%u (next will be %u)",
				        ch->GetName(), victim->GetName(),
				        serverComboIndex, ch->GetComboIndex());

				// ============================================================
				// VALIDATE VICTIM POSITION (optional anti-rangehack)
				// ============================================================

				long victimServerX = victim->GetX();
				long victimServerY = victim->GetY();

				if (packMelee->lStartX != 0 || packMelee->lStartY != 0)
				{
					// Calculate distance between client's victim position and server's victim position
					long deltaX = victimServerX - packMelee->lStartX;
					long deltaY = victimServerY - packMelee->lStartY;
					long distSq = deltaX * deltaX + deltaY * deltaY;

					// Allow larger distance for victim (they can be further away)
					const long MAX_VICTIM_DISTANCE_SQ = 2000 * 2000;

					if (distSq > MAX_VICTIM_DISTANCE_SQ)
					{
						sys_err("ATTACK_FULL: Victim position mismatch! Client=(%ld,%ld) Server=(%ld,%ld) Distance=%ld",
						        packMelee->lStartX, packMelee->lStartY,
						        victimServerX, victimServerY, (long)sqrt(distSq));
						// Continue anyway but log for monitoring
					}
				}

				// ============================================================
				// EXECUTE ATTACK (apply damage)
				// ============================================================

				ch->Attack(victim, packMelee->bType);

				// ============================================================
				// PREPARE SERVER-AUTHORITATIVE ATTACK BROADCAST
				// ============================================================

				TPacketGCAttack attackPacket;
				attackPacket.bHeader = HEADER_GC_ATTACK;
				attackPacket.dwAttackerVID = ch->GetVID();
				attackPacket.dwVictimVID = victim->GetVID();
				attackPacket.bType = packMelee->bType;

				// Use server's victim position (authoritative)
				attackPacket.lStartX = victimServerX;
				attackPacket.lStartY = victimServerY;

				// ============================================================
				// SERVER CALCULATES KNOCKBACK (NOT CLIENT!)
				// CRITICAL FIX: Ignore client destination coords
				// ============================================================

				// Knockback happens ONLY on 4th hit (combo index 3)
				if (serverComboIndex == 3)
				{
					sys_err("DEBUG_ATTACK_7: ═══ 4TH HIT - CHECKING KNOCKBACK IMMUNITY ═══");

					// ============================================================
					// KNOCKBACK IMMUNITY CHECK (2 second cooldown)
					// Victim is immune to knockback for 2 seconds after receiving one
					// ============================================================

					DWORD dwLastKnockbackTime = victim->GetLastKnockbackTime();
					DWORD dwTimeSinceLastKnockback = dwNow - dwLastKnockbackTime;
					const DWORD KNOCKBACK_IMMUNITY_MS = 2000; // 2 seconds immunity

					sys_err("DEBUG_ATTACK_7:   Victim last knockback: %u ms ago", dwTimeSinceLastKnockback);
					sys_err("DEBUG_ATTACK_7:   Immunity duration: %u ms", KNOCKBACK_IMMUNITY_MS);

					if (dwLastKnockbackTime > 0 && dwTimeSinceLastKnockback < KNOCKBACK_IMMUNITY_MS)
					{
						// VICTIM IS IMMUNE - Skip knockback
						sys_err("DEBUG_ATTACK_7:   ⚠️ KNOCKBACK BLOCKED - Victim is IMMUNE! (%u ms remaining)",
						        KNOCKBACK_IMMUNITY_MS - dwTimeSinceLastKnockback);

						attackPacket.lDestX = victimServerX;
						attackPacket.lDestY = victimServerY;
						attackPacket.dwDuration = 0;

						sys_log(0, "COMBO: Hit 4 but victim has knockback immunity (%ums remaining)",
						        KNOCKBACK_IMMUNITY_MS - dwTimeSinceLastKnockback);
					}
					else
					{
						// VICTIM CAN RECEIVE KNOCKBACK
						sys_err("DEBUG_ATTACK_7:   ✅ KNOCKBACK ALLOWED - Calculating...");

						// SERVER calculates knockback direction and distance
						long attackerX = ch->GetX();
						long attackerY = ch->GetY();

						sys_err("DEBUG_ATTACK_7:   Attacker pos: (%ld, %ld)", attackerX, attackerY);
						sys_err("DEBUG_ATTACK_7:   Victim pos: (%ld, %ld)", victimServerX, victimServerY);

						// Calculate direction vector from attacker to victim
						float fDirX = (float)(victimServerX - attackerX);
						float fDirY = (float)(victimServerY - attackerY);
						float fLength = sqrt(fDirX * fDirX + fDirY * fDirY);

						sys_err("DEBUG_ATTACK_7:   Direction vector: (%.2f, %.2f) length=%.2f", fDirX, fDirY, fLength);

						if (fLength > 0.1f) // Prevent division by zero
						{
							// Normalize direction vector
							fDirX /= fLength;
							fDirY /= fLength;

							sys_err("DEBUG_ATTACK_7:   Normalized: (%.4f, %.4f)", fDirX, fDirY);

							// Apply knockback distance (configurable)
							const long KNOCKBACK_DISTANCE = 500;

							// Calculate destination (SERVER-AUTHORITATIVE!)
							attackPacket.lDestX = victimServerX + (long)(fDirX * KNOCKBACK_DISTANCE);
							attackPacket.lDestY = victimServerY + (long)(fDirY * KNOCKBACK_DISTANCE);
							attackPacket.dwDuration = 500; // 500ms knockback duration

							sys_err("DEBUG_ATTACK_7:   KNOCKBACK CALCULATED:");
							sys_err("DEBUG_ATTACK_7:     FROM: (%ld, %ld)", victimServerX, victimServerY);
							sys_err("DEBUG_ATTACK_7:     TO:   (%ld, %ld)", attackPacket.lDestX, attackPacket.lDestY);
							sys_err("DEBUG_ATTACK_7:     Distance: %ld units", KNOCKBACK_DISTANCE);
							sys_err("DEBUG_ATTACK_7:     Duration: %u ms", attackPacket.dwDuration);

							// Apply knockback to victim on server (sync tracking)
							sys_err("DEBUG_ATTACK_8: Calling victim->BlendSync(%ld, %ld, %u)",
							        attackPacket.lDestX, attackPacket.lDestY, attackPacket.dwDuration);
							victim->BlendSync(attackPacket.lDestX, attackPacket.lDestY, attackPacket.dwDuration);
							sys_err("DEBUG_ATTACK_8: victim->BlendSync() returned");

							// SET KNOCKBACK IMMUNITY - Victim immune for next 2 seconds
							victim->SetLastKnockbackTime(dwNow);
							sys_err("DEBUG_ATTACK_8: ✅ Knockback immunity set until %u", dwNow + KNOCKBACK_IMMUNITY_MS);

							sys_log(0, "COMBO: KNOCKDOWN on hit 4! Server calculated knockback (%ld,%ld)->(%ld,%ld) dist=%ld",
							        victimServerX, victimServerY,
							        attackPacket.lDestX, attackPacket.lDestY,
							        KNOCKBACK_DISTANCE);
						}
						else
						{
							// Attacker and victim at same position - no knockback
							attackPacket.lDestX = victimServerX;
							attackPacket.lDestY = victimServerY;
							attackPacket.dwDuration = 0;

							sys_log(0, "COMBO: Hit 4 but attacker/victim same position - NO knockback");
						}
					}
				}
				else
				{
					// NO knockback on hits 1-3
					attackPacket.lDestX = victimServerX;
					attackPacket.lDestY = victimServerY;
					attackPacket.dwDuration = 0;

					sys_log(0, "COMBO: Hit %u - NO knockback", serverComboIndex + 1);
				}

				// Use SERVER's combo index (authoritative)
				attackPacket.bComboIndex = serverComboIndex;

				sys_err("DEBUG_ATTACK_9: ═══ BROADCASTING TPacketGCAttack ═══");
				sys_err("DEBUG_ATTACK_9:   bHeader: %d", attackPacket.bHeader);
				sys_err("DEBUG_ATTACK_9:   dwAttackerVID: %u (%s)", attackPacket.dwAttackerVID, ch->GetName());
				sys_err("DEBUG_ATTACK_9:   dwVictimVID: %u (%s)", attackPacket.dwVictimVID, victim->GetName());
				sys_err("DEBUG_ATTACK_9:   bType: %d", attackPacket.bType);
				sys_err("DEBUG_ATTACK_9:   lStartX: %ld, lStartY: %ld (victim START)", attackPacket.lStartX, attackPacket.lStartY);
				sys_err("DEBUG_ATTACK_9:   lDestX: %ld, lDestY: %ld (victim DEST)", attackPacket.lDestX, attackPacket.lDestY);
				sys_err("DEBUG_ATTACK_9:   dwDuration: %u ms", attackPacket.dwDuration);
				sys_err("DEBUG_ATTACK_9:   bComboIndex: %u", attackPacket.bComboIndex);

				// ============================================================
				// AREA BROADCAST - Send to ALL nearby players
				// ============================================================

				// PacketAround broadcasts to all players in the area, including:
				// - Attacker
				// - Victim
				// - All other players nearby (observers)
				sys_err("DEBUG_ATTACK_10: Calling PacketAround (size=%u)", sizeof(TPacketGCAttack));
				ch->PacketAround(&attackPacket, sizeof(TPacketGCAttack), ch);
				sys_err("DEBUG_ATTACK_10: PacketAround returned");

				sys_log(0, "ATTACK_FULL: Broadcasted to area (attacker=%s victim=%s combo=%u knockback=%s)",
				        ch->GetName(), victim->GetName(), serverComboIndex,
				        (serverComboIndex == 3 && attackPacket.dwDuration > 0) ? "YES" : "NO");

				sys_err("═══════════════════════════════════════════════════════════");
				sys_err("DEBUG_ATTACK_FINAL: CInputMain::Attack() COMPLETE");
				sys_err("═══════════════════════════════════════════════════════════");
			}
			break;
