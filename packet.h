...
// ���� - Client to Server Attack Packet
typedef struct command_attack
{
	BYTE	bHeader;
	BYTE	bType;
	DWORD	dwVID;				// Victim VID
	BYTE	bCRCMagicCubeProcPiece;
	BYTE	bCRCMagicCubeFilePiece;

	// PHASE 2 FULL: Victim position tracking
	long	lStartX;			// Victim position BEFORE attack
	long	lStartY;
	long	lDestX;				// Victim position AFTER knockback
	long	lDestY;
	DWORD	dwDuration;			// Knockback duration in ms
	DWORD	dwTimestamp;		// Attack timestamp (anti-cheat)
	BYTE	bComboIndex;		// Combo index (0,1,2,3)
} TPacketCGAttack;
...
// PHASE 2 FULL: Server to Client Attack Broadcast
typedef struct packet_attack
{
	BYTE	bHeader;			// HEADER_GC_ATTACK
	DWORD	dwAttackerVID;		// Who attacked
	DWORD	dwVictimVID;		// Who was hit
	BYTE	bType;				// Attack/skill type

	// Server-validated victim position and knockback
	long	lStartX;			// Victim start position (server-validated)
	long	lStartY;
	long	lDestX;				// Victim dest position (server-calculated)
	long	lDestY;
	DWORD	dwDuration;			// Knockback duration in ms
	BYTE	bComboIndex;		// Combo index
} TPacketGCAttack;
