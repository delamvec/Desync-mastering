...
bool CActorInstance::InputComboAttackCommand(float fDirRot)
{
	...
	else if (m_pkCurRaceMotionData->IsComboInputTimeData())
	{
		...
		if (fElapsedTime > m_pkCurRaceMotionData->GetNextComboTime()) // 콤보 중도 시간 안 지났다
		{
			//Tracen("즉시 콤보 시작");
			// args : BlendingTime
			__RunNextCombo();
		...
		else if (fElapsedTime > m_pkCurRaceMotionData->GetComboInputStartTime()) // 선 입력 시간 안에 왔음..
		{
			//Tracen("선 입력 시작");
			m_isPreInput = TRUE;
			return false;
	...
	{
		float fElapsedTime = GetAttackingElapsedTime();
		if (fElapsedTime > m_pkCurRaceMotionData->GetMotionDuration()*0.9f) // 콤보 중도 시간 안 지났다
		{
			//Tracen("즉시 콤보 시작");
			// args : BlendingTime
			__RunNextCombo();
...
void CActorInstance::ComboProcess()
{
	...
	else
	{
		...
		if (!IsUsingSkill())	// m_isNextPreInput을 활성할 때라던지 제한하는 목적
		if (m_isNextPreInput)	// 활때라던지 스킬이 캔슬 되는거 이거 때문에
		{
			__RunNextCombo();
...
void CActorInstance::__OnEndCombo()
{
	...
	// 여기서 콤보를 초기화 해서는 안된다.
	// 콤보가 초기화 되는 시점 정확히 콤보의 마지막 Motion 이 자동으로 Wait 상태 넘어갈 때이다.
}
...
bool IS_PARTY_HUNTING_RACE(unsigned int vnum)
{
	...
	if (vnum >= 8000 && vnum <= 8112) // 몹종류
		return true;

	if (vnum >= 2400 && vnum <  5000) // 천마 전사 같은 것들
		return true;
...
void CActorInstance::__ProcessDataAttackSuccess(const NRaceData::TAttackData & c_rAttackData, CActorInstance & rVictim, const D3DXVECTOR3 & c_rv3Position, UINT uiSkill, BOOL isSendPacket)
{
	...
	if (IS_PARTY_HUNTING_RACE(rVictim.GetRace()))
	{
		...
		if (m_isMain) // #0000794: [M2KR] 무적시간 - 뒤쪽은 몹에 타격 적용에 대해 자신 타격에 대한것을 체크한다
			rVictim.m_fInvisibleTime = CTimer::Instance().GetCurrentSecond() + c_rAttackData.fInvisibleTime;
	}
	else // 파티 사냥 몬스터가 아닌 것만 적용
	{
		rVictim.m_fInvisibleTime = CTimer::Instance().GetCurrentSecond() + c_rAttackData.fInvisibleTime;
	}
	...
	// 2004.08.03.myevan.빌딩이나 문에 때릴 타격 효과는 붙이지 않는다
...
// ============================================================
// FIXED: Modified SetBlendingPosition - now passes current position + delta
// ============================================================
void CActorInstance::SetBlendingPosition(const TPixelPosition & c_rPosition, float fBlendingTime)
{
	...
	GetPixelPosition(&currentPos);

	// Calculate delta
	TPixelPosition deltaPos;
	deltaPos.x = c_rPosition.x - m_x;
	deltaPos.y = c_rPosition.y - m_y;
	deltaPos.z = 0;

	// Pass both current and delta to SetLastPosition
	m_PhysicsObject.SetLastPosition(currentPos, deltaPos, fBlendingTime);
}
...
// ============================================================
// FIXED: GetBlendingPosition - returns FINAL position, not current + delta
// ============================================================
void CActorInstance::GetBlendingPosition(TPixelPosition * pPosition)
{
	if (m_PhysicsObject.isBlending())
	{
		// Return FINAL position (not current + delta!)
		m_PhysicsObject.GetFinalPosition(pPosition);
	}
	else
	{
		// Not blending - return current position
